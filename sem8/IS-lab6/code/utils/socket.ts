...
export function create(teamName: string, onMessage: (msg: Buffer) => void, goalie: boolean = false, version: number = 7): Socket {
    //Создание сокета
    ...
    socket.sendMsg(`(init ${teamName} ${goalie ? '(goalie) ' : ''}(version ${version}))`); //подключение игрока к серверу
    return socket;
}
