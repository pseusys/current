...
function get_datas (file) {
  const dataset = get_dataset(file);
  const datas = [];
  for (const data in dataset) datas.push(dataset[data]);
  return datas;
}
...
module.exports.add_broker = function (username, password) {
  const broker = { name: username, money: 0, isAdmin: false, actions: [] };
...
module.exports.get_action = function (id) { return get_data(id, actions_file); }
