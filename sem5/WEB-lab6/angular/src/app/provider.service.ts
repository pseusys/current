...
export class Broker {
  ...
  constructor(name?: string, money?: number, isAdmin?: boolean, actions?: UserAction[]) {
...
export class Settings {
  ...
  constructor(startDate: Date, endDate: Date, interval: number, law: string) {
...
export class ProviderService {
  ...
  async get_brokers(): Promise<Broker[]> {
    try {
      return await this.http.get<Broker[]>(this.cors + '/users', { headers: this.headers }).toPromise().then((data) => {
        const brokers = [];
        for (const broker of data) { brokers.push(new Broker(broker.name, broker.money, broker.isAdmin, broker.actions)); }
        return brokers;
      });
    } catch (e) {
  ...
  async create_user(username: string, password: string): Promise<Broker> {
    ...
    try {
      return await this.http.post<Broker>(this.cors + '/signin', params).toPromise().then((data) => {
        ...
        return new Broker(data.name, data.money, data.isAdmin, data.actions);
      });
    } catch (e) {
  ...
  async get_settings(): Promise<Settings> {
    try {
      return await this.http.get<Settings>(this.cors + '/settings', { headers: this.headers }).toPromise().then((data) => {
        return new Settings(data.tradeStart, data.tradeEnd, data.interval, data.law);
      });
    } catch (e) { return new Promise<Settings>(() => new Settings(new Date(), new Date(), 0, 'normal')); }
  }
  ...
  async set_settings(settings: Settings): Promise<Settings> {
    ...
    try {
      ...
      alert(e.error);
      return new Promise<Settings>(() => new Settings(new Date(), new Date(), 0, 'normal'));
    }
  }
}
