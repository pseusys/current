...
function set_users(users) {
    ...
    fs.writeFileSync(path.join(__dirname, "../storage-bravo/users.json"), user_string);
}

module.exports.get_users = function () {
    const user_book = fs.readFileSync(path.join(__dirname, "../storage-bravo/users.json"));
...
function get_passes() {
    const passwords = fs.readFileSync(path.join(__dirname, "../storage-bravo/passwords.json"));
    return JSON.parse(passwords.toString());
}

function set_passes(passes) {
    const passwords = JSON.stringify(passes);
    fs.writeFileSync(path.join(__dirname, "../storage-bravo/passwords.json"), passwords);
}
