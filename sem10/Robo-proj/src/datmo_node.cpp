...
public:
    ...
    void perform_clustering()
    {
        ...
        for (int loop = 1; loop < nb_beams; loop++) // loop over all the hits
            ...
            { // the current hit doesnt belong to the same hit
                ...
                nb_dynamic += dynamic[loop - 1] ? 1 : 0;

                end = loop - 1;

                cluster_size[nb_cluster] = distancePoints(current_scan[start], current_scan[end]);
                cluster_middle[nb_cluster] = current_scan[(end - start) / 2 + start];
                ...
                // textual display
                ROS_INFO("cluster[%i] = (%f, %f): hit[%i](%f, %f) -> hit[%i](%f, %f), size: %f, dynamic: %i", nb_cluster,
        ...
        cluster[nb_beams - 1] = nb_cluster;
        nb_dynamic += dynamic[nb_beams - 1] ? 1 : 0;
    ...
    void track_a_moving_person()
    {
        ...
        {
            float current_dist = distancePoints(moving_person_tracked, person_detected[loop_detection]); // EUCLIDIAN DISTANCE between the moving_person_tracked and the current_person_detected
