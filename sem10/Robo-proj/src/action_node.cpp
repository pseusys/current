...
void update() {
    ...
    if ( init_odom && init_obstacle ) { //wait for the initialization of odometer and detect_obstacle_node
        ...
        if ( !init_obstacle )
        {
            ...
            ROS_WARN("launch: rosrun follow_me obstacle_detection_node");
        }
...
void compute_rotation()
{
    ...
    rotation_done = (abs(rotation_done) < M_PI) * rotation_done + (rotation_done > M_PI) * (rotation_done - 2 * M_PI) + (rotation_done < - 2 * M_PI) + (rotation_done < - M_PI) * (2 * M_PI + rotation_done);
    ...
    cond_rotation = abs(error_rotation) > error_rotation_threshold; // cond_rotation is used to control if we stop or not the pid
    ...
    if ( cond_rotation )
    {
        ...
        rotation_speed = rotation_speed > rotation_speed_max ? rotation_speed_max : rotation_speed;
    }
...
void compute_translation()
{
    ...
    cond_translation = abs(error_translation) > error_translation_threshold; // cond_translation is used to control if we stop or not the pid for translation
	...
	float error_derivation_translation = error_translation - error_previous_translation;
        ...
        translation_speed = kpr * error_translation + kir * error_integral_translation + kdr * error_derivation_translation;
        translation_speed = translation_speed > translation_speed_max ? translation_speed_max : translation_speed;
...
void combine_rotation_and_translation()
{
    ...
    float coef_rotation = abs(rotation_speed) / rotation_speed_max;
    if ( coef_rotation >= 1 )
        coef_rotation = 1;
...
//DO NOT MODIFY THIS FUNCTION
    ...
    //DO NOT REMOVE
    if ( translation_speed > translation_speed_max )
        translation_speed = translation_speed_max;
