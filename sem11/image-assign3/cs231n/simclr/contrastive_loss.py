...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_sim_k[k] = 0.0
        exp_sim_k_N[k + N] = 0.0
        loss_k = -torch.log(torch.exp(pos_sim / tau) / exp_sim_k.sum())
        loss_k_N = -torch.log(torch.exp(pos_sim / tau) / exp_sim_k_N.sum())
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_products = torch.sum(out_left * out_right, dim=1, keepdim=True)
    norms_left = torch.linalg.norm(out_left, dim=1, keepdim=True)
    norms_right = torch.linalg.norm(out_right, dim=1, keepdim=True)
    pos_pairs = dot_products / (norms_left * norms_right)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.T) / (norms @ norms.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = torch.cat([out_left, out_right], dim=0)
    pos_sim = torch.sum(pos_pairs[:N] * pos_pairs[N:], dim=1)
    ...
    numerator = torch.exp(pos_sim / tau).unsqueeze(1)
